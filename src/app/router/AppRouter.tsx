...
const fallback = <div className="grid min-h-[50vh] place-items-center"><Spinner label="Loading page" /></div>
const load = (page: ReactNode) => <Suspense fallback={fallback}>{page}</Suspense>

const router = createBrowserRouter([
  { element: <MainLayout />, errorElement: <RouteError />, children: [
    { index: true, element: load(<HomePage />) }, { path: 'library', element: load(<LibraryPage />) },
    { path: 'upload', element: load(<UploadPage />) }, { path: 'processing/:bookId', element: load(<ProcessingPage />) },
    { path: 'settings', element: load(<SettingsPage />) },
  ] },
  { element: <ReaderLayout />, errorElement: <RouteError />, children: [{ path: 'reader/:bookId', element: load(<ReaderPage />) }] },
